...
class MovingSprite(AnimatedSprite):
    ...
    def check_border(self):
        if self.move_dir == 'x':
            ...
            if self.rect.top >= self.end_pos[1] and self.direction.y == 1:
                self.direction.y = -1
                self.rect.top = self.end_pos[1]
            elif self.rect.bottom <= self.start_pos[1] and self.direction.y == -1:
                self.direction.y = 1
...
class Cloud(Sprite):
    def __init__(self, pos, surg, groups, z = Z_LAYERS['clouds']):
        super().__init__(pos, surg, groups, z)
